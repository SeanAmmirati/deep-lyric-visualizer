...
def create_imagenet_yaml(output_filename='image_classes.yml'):

    temp_dict = {0: 'tench, Tinca tinca',
...
def dict_assign(obj, d):

    for k, v in d.items():
        if not hasattr(obj, k):
            logger.debug(f'No attribute for object {obj} named {k}.')
            logger.debug(f'Setting attribute {k} to {v}')
            setattr(obj, k, v)
        else:
            logger.debug(
                f'Attribute for object {obj} already exists with value {getattr(obj, k)}.')
            logger.debug(f'Ignoring key: {k}, value: {v}')


def find_first_file_with_ext(dir_path, ext):
    match_file = None
    for f_name in os.listdir(dir_path):
        f_ext = os.path.splitext(f_name)[1]
        if f_ext == f'.{ext}':
            logger.debug(
                f'Found {ext} file {f_name}. Using this as the lyric source.')
            match_file = os.path.join(dir_path, f_name)
            break
        else:
            logger.debug(
                f'File {f_name} does not have {ext} extension. Skipping.')

    if match_file:
        logger.debug(f'Returning found file {match_file}')
    else:
        logger.debug(
            f'Could not find file with {ext} in {dir_path}. Returning None')
